<template>
  ...
  <vee-form :validation-schema="schema" @submit="register" :initial-values="userData">
    ...
    <div class="mb-3">
      ...
      <div>
        <vee-field type="radio" value="listener" name="userType" />
        <label for="listener">Listener</label>
      </div>
      <div>
        <vee-field type="radio" value="artist" name="userType" />
  ...
  data() {
    return {
      schema: {
        ...
        password: 'required|min:9|max:100|excluded:passwords',
        confirm_password: 'password_missmatch:@password',
        country: 'required|country_excluded:Tunisia',
        userType: 'required',
        tos: 'tos'
      },
      userData: {
      ...
      reg_alert_message: 'Please wait! Your accout is being created.'
    };
  },
  methods: {
    ...
    async register(values) {
      ...
      this.reg_alert_message = 'Please wait! Your accout is being created.';
      try {
        await this.createUser(values);
      } catch (error) {
        this.reg_in_submission = false;
        this.reg_alert_variant = 'bg-red-500';
        this.reg_alert_message = 'Oops! Something went wrong.';
        console.log(error);
        return;
      }
      this.isLoggedIn = true;
      this.reg_alert_variant = 'bg-green-500';
      this.reg_alert_message = 'Great! Your accout has been created.';
